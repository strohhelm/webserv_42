...
//             {
//                 acceptNewConnection(fdIndex); // Accept new connection and add it to poll
//             }
//             else
//             {
//                 // Check if the client sent a request
//                 if (readDataFromClient(fdIndex) < 0)
//                 {
//                     // If read failed, it's a hang-up or error, handle appropriately
//                     closeConnection(fdIndex); 
//                 }
//                 else
//                 {
//                     // Process HTTP request (handle parsing and response)
//                     // processRequest(fdIndex);
// 					handler(fdIndex);

                    
//                     // If the connection is persistent (HTTP/1.1 Keep-Alive), don't close the socket
//                     if (!shouldCloseConnection(fdIndex)) 
//                     {
//                         // You may want to reset or handle timeouts based on keep-alive headers
//                         resetIdleTimeout(fdIndex); 
//                     }
//                     else
//                     {
//                         closeConnection(fdIndex);  // Close if it's a `Connection: close` request or timeout
//                     }
//                 }
//             }
//         }
...
//         if (_poll_fds[fdIndex].revents & POLLOUT)
//         {
//             handler(fdIndex);  // Handle sending data (e.g., HTTP response)
//         }

//         // Handle errors or hang-ups
//         if (_poll_fds[fdIndex].revents & (POLLERR | POLLHUP))
//         {
//             std::cout << "Error or hang-up detected on socket " << _poll_fds[fdIndex].fd << std::endl;
//             closeConnection(fdIndex);  // Close the connection on error
//         }
